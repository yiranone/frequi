  ...
  <form class="flex flex-row" @submit.prevent="saveNewName">
    ...
    <div class="mt-auto flex gap-1 ms-1" :class="alignVertical ? 'flex-col' : 'flex-row'">
      ...
      <template v-if="mode !== EditState.None">
        <Button
          size="small"
          :title="mode === EditState.Editing ? `保存${editableName}` : `确认${editableName}`"
          severity="primary"
          @click="saveNewName"
        >
          <template #icon>
        ...
        <Button size="small" title="取消" severity="secondary" @click="abort">
          <template #icon>
            <i-mdi-close />
          </template>
        </Button>
