...
<template>
  <div class="flex flex-row pt-1 me-1 relative" style="height: calc(100vh - 60px)">
    ...
    <div class="flex flex-col w-full">
      <h2 class="ms-5 text-3xl font-bold">回测中心</h2>
      <p v-if="!botStore.activeBot.canRunBacktest">
        机器人必须运行在 Webserver 模式下，才能启用回测功能。
      </p>
      <div class="w-full">
        <Tabs v-model:value="btFormMode" lazy>
          <TabList>
            <Tab
              ...
              ><i-mdi-cloud-download class="me-2" />加载结果</Tab
            >
            <Tab
              class="flex items-center"
              value="run"
              :disabled="!botStore.activeBot.canRunBacktest"
              ><i-mdi-run-fast class="me-2" />运行回测</Tab
            >
            <Tab
              id="bt-analyze-btn"
              class="flex items-center"
              ...
              ><i-mdi-table-eye class="me-2" />结果分析</Tab
            >
            <Tab
              ...
              ><i-mdi-compare-horizontal class="me-2" />结果对比</Tab
            >
            <Tab class="flex items-center" value="visualize-summary" :disabled="!hasBacktestResult"
              ><i-mdi-chart-bell-curve-cumulative class="me-2" />摘要可视化</Tab
            >
            <Tab class="flex items-center" value="visualize" :disabled="!hasBacktestResult"
              ><i-mdi-chart-timeline-variant-shimmer class="me-2" />结果可视化</Tab
            >
          </TabList>
          <TabPanels>
